...
- `git remote add origin <URL_РЕПОЗИТОРИЯ>` и `git push -u origin main`.
- При необходимости — настроить продовые значения `.env` (реальные `DATABASE_URL`,
  `REDIS_URL`, `BASE_URL`, `CORS_ORIGIN`) на хостинге.
